...
const canvas = document.getElementsByID('creamyCanvas1')[0];
...
function initFramebuffers() {
  ...
  if (density == null)
  density = createDoubleFBO(dyeWidth, dyeHeight, rgba.internalFormat, rgba.format, texType, filtering);else

  density = resizeDoubleFBO(density, dyeWidth, dyeHeight, rgba.internalFormat, rgba.format, texType, filtering);

  if (velocity == null)
  velocity = createDoubleFBO(simWidth, simHeight, rg.internalFormat, rg.format, texType, filtering);else
...
function getResolution(resolution) {
  ...
  return { width: max, height: min };else
