...
createCoffeeDrip(canvas[0])
createCoffeeDrip(canvas[1])

function createCoffeeDrip (el) {
    ...
    let config = {
        ...
        VELOCITY_DISSIPATION: 0.955,
        PRESSURE_DISSIPATION: 0.69,
    ...
    const { gl, ext } = getWebGLContext(el);
    ...
    function render (target) {
        ...
        if (target == null && config.TRANSPARENT) {
            ...
            gl.uniform1f(backgroundProgram.uniforms.aspectRatio, el.width / el.height);
            blit(null);
        }
    ...
    function splat (x, y, dx, dy, color) {
        ...
        gl.uniform1f(splatProgram.uniforms.aspectRatio, el.width / el.height);
        gl.uniform2f(splatProgram.uniforms.point, x / el.width, 1.0 - y / el.height);
    ...
    function multipleSplats (amount) {
        for (let i = 0; i < amount; i++) {
            ...
            color.g *= 10.0;
            color.b *= 10.0;
            const x = el.width * Math.random();
            const y = el.height * Math.random();
